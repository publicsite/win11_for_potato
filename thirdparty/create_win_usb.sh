...
REQUIRED_COMMANDS=("rsync" "/sbin/parted" "/usr/sbin/wipefs" "/usr/sbin/mkfs.vfat" "/usr/sbin/mkfs.ntfs" "udisksctl" "sha256sum" "mount" "umount" "mkdir" "cp" "sync" "mktemp")
...
then
    ...
    /usr/sbin/wipefs -a $USB_BLOCK

    /sbin/parted $USB_BLOCK mklabel gpt
    /sbin/parted $USB_BLOCK mkpart BOOT fat32 0% 1GiB
    /sbin/parted $USB_BLOCK mkpart INSTALL ntfs 1GiB 100%
    /sbin/parted $USB_BLOCK unit B print
    ...
    echo "Copying to USB..."
    /usr/sbin/mkfs.vfat -n BOOT ${USB_BLOCK}1
    ...
    /usr/sbin/mkfs.ntfs --quick -L INSTALL ${USB_BLOCK}2
